...
from cshogi.dlshogi import make_input_features, FEATURES1_NUM, FEATURES2_NUM, make_move_label
...
# dlshogi_ネットワーク
DEFAULT_NETWORK = "resnet10_swish"
...
# 接待係数を探索した深さに応じて変更
def adjusted_settai(a):
    length = len(a)
    if length % 2 == 0:
        return 1.0  # 偶数のときは最大値

    step = 0.04  # 0.8 → 0.36 まで13段階
    raw_value = 0.9 - ((length - 1) // 2) * step
    return max(raw_value, 0.36)

def reverse_adjusted_settai(a):
    length = len(a)
    if length % 2 == 0:
        return 1.0
    ...
    step_count = (length - 1) // 2
    raw_value = min_val + step * step_count
    return min(raw_value, max_val)

class MCTSPlayer(BasePlayer):
    # USIエンジンの名前
    name = 'python-dlshogi2'
    # デフォルトチェックポイント
    DEFAULT_MODELFILE = '/workspace/model/model_resnet10_swish-072'
    def __init__(self):
        super().__init__()
        # チェックポイントのパス
        ...
        # 接待係数
        self.settai = DEFALUT_SETTAI / 100
    ...
    def usi(self):
        ...
        print('option name network type string default ' + self.network)
        ...
        print('option name settai type spin default ' + str(DEFALUT_SETTAI) + ' min 0 max 100')
        print('option name debug type check default false')
        print('option name aiming_move type check default false')
    ...
    def setoption(self, args):
        if args[1] == 'modelfile':
            ...
            self.settai = int(args[3]) / 100
        elif args[1] == 'network':
            self.network = args[3]
        elif args[1] == 'debug':
            self.debug = args[3] == 'true'
        elif args[1] == 'aiming_move':
            self.aiming_move = args[3] == 'true'

    # モデルのロード
    def load_model(self):
        self.model = policy_value_network(self.network)
    ...
    def go(self):
        ...
        if self.debug:
            ...
            print(f"keiro_len:{self.debug_keiro}")
    ...
    def uct_search(self, board, current_node, trajectories):
        ...
        if self.aiming_move:
            # if len(trajectories) % 2 == 1:
            #     # 通常のUCB値が最大の手をもとめる
            #     # ルートノードで手番側
            #     next_index = self.select_max_ucb_child(current_node, root_turn=True)
            # else:
            #     # 接待係数を用いたUCB値の手をもとめる
            #     # ルートノードで相手番
            #     next_index = self.select_max_ucb_child(current_node, root_turn=False)
            
            if len(trajectories) > self.debug_keiro:
                self.debug_keiro = len(trajectories)
            next_index = self.select_max_ucb_child(current_node, trajectories)
        else:
            # UCB値が最大の手を求める
            next_index = self.select_max_ucb_child(current_node, trajectories=None)
        # 選んだ手を着手
    ...
    # UCB値が最大の手を求める
    def select_max_ucb_child(self, node, trajectories=None):
        ...
        if trajectories == None:
            ...
            settai = reverse_adjusted_settai(trajectories)
            ucb_turn = len(trajectories)
            if ucb_turn % 2 == 1:
                ucb = 1.0 - abs(q - self.settai) / max(1.0 - self.settai, self.settai) + self.c_puct * u * node.policy
            else:
                ucb = q + self.c_puct * u * node.policy
            # ucb = 1.0 - abs(q - settai) / max(1.0 - settai, settai) + self.c_puct * u * node.policy
